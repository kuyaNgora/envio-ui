...
export const getSlides = (props: CarouselProps) => {
...
export const getItemsCount = (props: CarouselProps) => {
  return getSlides(props).length;
};

export const getItemsOffset = (props: CarouselProps) => {
...
export const getItemsInSlide = (itemsCount: number, props: CarouselProps) => {
  let itemsInSlide = 1;
...
export const createClones = (props: CarouselProps) => {
...
export const createAutowidthTransformationSet = (
  ...
  if (isElement(el)) {
    // TODO: refactoring
    const children: Array<HTMLElement | Element> = Array.from(
      el?.children || []
    );
    ...
    coords = children.reduce<ItemCoords[]>((acc, child, i) => {
      ...
      const { width = 0 } = getElementDimensions(
...
export const createDefaultTransformationSet = (
  ...
  coords = children.reduce<ItemCoords[]>((acc, i: any) => {
    let position = 0;
...
export const getElementFirstChild = (
  stageComponent: HTMLElement | null,
  cursor: number
) => {
...
export const getAutoheightProperty = (
  stageComponent: HTMLElement | null,
  props: CarouselProps,
  state: CarouselState
) => {
  const elementCursor = getElementCursor(props, state);
  const element = getElementFirstChild(
    stageComponent,
    elementCursor
  ) as HTMLElement;
...
export function animate(element: HTMLElement | null, options: Options) {
  ...
  if (element && isElement(element)) {
    element.style[
      "transition"
    ] = `transform ${animationDuration}ms ${animationEasingFunction} 0ms`;
    element.style["transform"] = `translate3d(${position}px, 0, 0)`;
  }
  return element;
}

export const getRenderWrapperStyles = (
  props: CarouselProps,
  state: CarouselState,
  element: HTMLElement | null
) => {
  const { paddingLeft, paddingRight, autoHeight, animationDuration } =
    props || {};
  const height = autoHeight
    ? getAutoheightProperty(element, props, state)
    : undefined;
...
export const getTransitionProperty = (options?: Transition): string => {
  const { animationDuration = 0, animationEasingFunction = "ease" } =
    options || {};
  return `transform ${animationDuration}ms ${animationEasingFunction} 0ms`;
};

export const getRenderStageStyles = (
  nextStyles: Options,
  currentStyles: Style
): Style => {
...
export const getRenderStageItemStyles = (i: number, state: CarouselState) => {
  const {
    transformationSet,
...
export const getTranslate3dProperty = (
  nextIndex: number,
...
export const getTouchmoveTranslatePosition = (
  deltaX: number,
  translate3d: number
) => {
