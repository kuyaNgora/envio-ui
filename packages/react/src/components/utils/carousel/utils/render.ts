...
export const getActiveSlideDotsLength = (
  slidesLength = 0,
  itemsInSlide = 1
) => {
...
export const getSlideIndexForNonMultipleItems = (
  activeIndex: number,
  itemsInSlide: number,
  slidesLength: number
) => {
...
export const getSlideIndexForMultipleItems = (
  ...
  if (isNextSlideDisabled || (activeIndex < itemsInSlide && activeIndex !== 0))
    return dotsLength;
  if (activeIndex === 0) {
...
export const getSlideItemInfo = (state: CarouselState) => {
  const {
    itemsInSlide,
    activeIndex,
    infinite,
  ...
  const isNextSlideDisabled =
    infinite === false && itemsCount - itemsInSlide <= activeIndex;
